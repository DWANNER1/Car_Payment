...
CREATE TABLE organizations (
    ...
    dms_config JSONB, -- { type, api_key, endpoint, client_id, client_secret }
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 2. Customer Tokens (Card-on-File)
...
CREATE TABLE transactions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id UUID REFERENCES organizations(id) ON DELETE CASCADE,
    user_id UUID, -- nullable for webhook/auto posts
    ...
    status VARCHAR(20) DEFAULT 'pending', -- captured, voided, refunded, settled, failed
    is_settled BOOLEAN DEFAULT FALSE,
    dms_posted BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Indexes for performance on single instance
